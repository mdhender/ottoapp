...
const getUserByClan = `-- name: GetUserByClan :one
SELECT user_id
FROM users
WHERE is_active = 1
  AND clan = ?1
...
// Fails if the user is not active.
func (q *Queries) GetUserByClan(ctx context.Context, clanNo string) (int64, error) {
	row := q.db.QueryRowContext(ctx, getUserByClan, clanNo)
	var user_id int64
	err := row.Scan(&user_id)
	return user_id, err
}
